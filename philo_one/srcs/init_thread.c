...
/*   Updated: 2021/05/27 19:28:03 by ehautefa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*launch_thread(void *arg)
{
	...
	while (ph->num_of_eat != 0 && check_alive(ph->env) == 0)
	{
		...
		printf("%-3d MS %d has lefted the %d fork\n", (int)(-get_time_in_ms(ph->env->start)
				+ get_time_in_ms(time)), ph->id + 1, ph->id);
		pthread_mutex_unlock(&ph->env->forks[(ph->id + 1) % ph->env->nb_forks]);
		printf("%-3d MS %d has lefted the %d fork\n", (int)(-get_time_in_ms(ph->env->start)
				+ get_time_in_ms(time)), ph->id + 1, (ph->id + 1) % ph->env->nb_forks);
		gettimeofday(&time, NULL);
	...
	while (env->alive == 0)
	{
		...
		while (i < env->nb_of_ph)
		{
			...
			{		
				...
				pthread_mutex_unlock(&env->m_alive);
				env->who_is_dead = i; 
				pthread_detach(env->ph[env->nb_of_ph].thread);
				// i = 0;
				// while (i < env->nb_of_ph)
				// {
